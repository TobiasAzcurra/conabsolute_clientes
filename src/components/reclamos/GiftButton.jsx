    ...
    const result = await runTransaction(firestore, async (transaction) => {
      ...
      if (!orderSnapshot.exists()) {
        console.error(
          ...
          `pedidos/${year}/${month}/${day}`
        );
        throw new Error(
          `No existen pedidos para la fecha ${day}/${month}/${year}`
        );
      }

      const pedidosDelDia = orderSnapshot.data()?.pedidos || [];
      const pedidoIndex = pedidosDelDia.findIndex(
        (pedido) => pedido.id === orderData.id
      );
      ...
      if (
        pedidosDelDia[pedidoIndex].reclamo?.gift &&
        pedidosDelDia[pedidoIndex].reclamo.gift.length > 0
      ) {
        throw new Error("Este pedido ya tiene vouchers de regalo asignados");
      }
      ...
      if (!voucherSnapshot.exists()) {
        ...
        throw new Error(
          "El documento de compensaciones no existe. Path: vouchers/Compensaciones"
        );
      }
    ...
    <div className="w-full">
      {hasExistingGift ? (
        ...
        <button
          ...
          disabled={loading}
          className="text-4xl w-full z-50 text-center mt-6 flex items-center justify-center bg-red-500 text-gray-100 rounded-3xl h-20 font-bold"
        >
          {loading ? (
      ...
      <AppleModal
        ...
        title="Tu regalo!"
        copy={true}
        textToCopy={`Mis codigos 2x1 de ANHELO son: ${generatedVouchers
          .map((v) => v.codigo)
          .join(", ")}`}
      >
        <div className="flex flex-col space-y-4">
          <p className="text-center text-xs text-gray-500 ">
            Aca hay 5 vouchers 2x1! Podes canjearlos en tus próximos pedidos en
            donde rellenas con los datos de envio.
          </p>
