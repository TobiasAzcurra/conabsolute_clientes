...
const FormCustom = ({ cart, total }) => {const navigate = useNavigate();
    ...
    const handleCouponChange = (index, value, setFieldValue) => {
        ...
        if (value.length < 5) {
            ...
            updatedVoucherStatus[index] = "El código debe tener exactamente 5 caracteres.";
        }
    ...
    const handleVoucherValidation = async (
        ...
        try {
            ...
            if (promoProducts.length > 0 && nonPromoProducts.length === 0) {
                const updatedVoucherStatus = [...voucherStatus];
                updatedVoucherStatus[index] = "No se pueden aplicar vouchers a productos en promoción.";
                setVoucherStatus(updatedVoucherStatus);
                return;
            }
            ...
            if (promoProducts.length > 0 && nonPromoProducts.length > 0) {
                const updatedVoucherStatus = [...voucherStatus];
                updatedVoucherStatus[index] = "El cupón se aplica solo a productos no promocionales.";
                setVoucherStatus(updatedVoucherStatus);
    ...
    const TimeSelector = ({ selectedHora, handleChange, setFieldValue }) => {
        ...
        return (
            <Field
                as="select"
                name="hora"
                className={`custom-select ${selectedHora === "" ? "text-gray-400" : "text-black"}`}
                value={selectedHora}
                onChange={(e) => {
                    handleChange(e);
            ...
            >
                <option value="" disabled>¿Quieres reservar para más tarde?</option>
                ...
                {({
                    ...
                    useEffect(() => {
                        couponCodes.forEach((code, index) => {
                            if (code.trim().length === 5 && voucherStatus[index] !== "¡Código válido!") {
                                handleVoucherValidation(index, code, couponCodes, setFieldValue);
                            }
                        });
					...
					return (
                        <Form>
                            <div className="flex flex-col mb-2">
                                ...
                                <div className="flex justify-center flex-col mt-7 items-center">
                                    ...
                                    <div className="flex flex-row w-full gap-2 mb-4">
                                        ...
                                        <button
                                            ...
                                            className={`h-20 flex-1 flex-col font-bold items-center flex justify-center rounded-lg ${
                                                values.deliveryMethod === "takeaway" ? "bg-black text-gray-100" : "bg-gray-300 text-black"
                                            }`}
                                        ...
                                        >
                                            <div className="flex flex-row items-center gap-2">
                                                <img
                                                    src={isologo}
                                                    className={`h-4 ${values.deliveryMethod === "takeaway" ? "invert brightness-0" : "brightness-0"}`}
                                                    alt=""
                                                />
                                    ...
                                    </div><div className={`w-full items-center rounded-3xl border-2 border-black transition-all duration-300`}>
                                        ...
                                        <div className={`flex flex-col items-center transition-all duration-300 ${values.deliveryMethod === "delivery" ? "border-t border-black border-opacity-20" : ""}`}>
                                            ...
                                            <div className="w-full">
                                                <ErrorMessage name="phone" render={(msg) => (<AppleErrorMessage>{msg}</AppleErrorMessage>)} />
                                            </div>
                                        </div>{/* Campo para reservar hora */}
                                        <div className="flex flex-row justify-between px-3 h-auto items-start border-t border-black border-opacity-20">
                                            <div className="flex flex-row items-center gap-2">
                                                ...
                                                <TimeSelector
                                                    ...
                                                    setFieldValue={setFieldValue}
                                                />
                                                <ErrorMessage name="hora" component="span" className="text-sm text-red-main font-coolvetica font-light mt-1" />
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    <div className="w-full items-center rounded-3xl border-2 border-black">
                                        ...
                                        <div className="flex flex-col">
                                            {couponCodes.map((coupon, index) => (
                                                <div key={index} className={`flex flex-col w-full transition-all duration-300 ${index !== 0 ? "border-t border-black border-opacity-20" : ""}`}><div className="flex flex-row gap-2 px-3 items-center">
												<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="h-6">
													<path fillRule="evenodd" d="M1.5 6.375c0-1.036.84-1.875 1.875-1.875h17.25c1.035 0 1.875.84 1.875 1.875v3.026a.75.75 0 0 1-.375.65 2.249 2.249 0 0 0 0 3.898.75.75 0 0 1 .375.65v3.026c0 1.035-.84 1.875-1.875 1.875H3.375A1.875 1.875 0 0 1 1.5 17.625v-3.026a.75.75 0 0 1 .374-.65 2.249 2.249 0 0 0 0-3.898.75.75 0 0 1-.374-.65V6.375Z" clipRule="evenodd" />
												</svg>

												<MyTextInput
													name={`couponCode${index}`}
													type="text"
													placeholder={index === 0 ? "¿Tenes algun cupón?" : "¿Tenes otro cupón?"}
													value={couponCodes[index]}
													onChange={(e) => {
														handleCouponChange(index, e.target.value, setFieldValue);
													}}
													className="bg-transparent px-0 h-10 text-opacity-20 outline-none w-full"
												/>

												{isValidating[index] ? (
													<div className="inline-block h-4 w-4 animate-spin rounded-full border-4 border-solid border-current border-e-transparent align-[-0.125em] text-surface motion-reduce:animate-[spin_1.5s_linear_infinite] text-black" role="status">
														<span className="sr-only">Loading...</span>
													</div>
												) : voucherStatus[index] === "¡Código válido!" ? (
													<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="green" className="h-6">
														<path fillRule="evenodd" d="M10.828 16.172a.75.75 0 0 1-1.06 0L5.47 11.875a.75.75 0 0 1 1.06-1.06l3.298 3.297 6.364-6.364a.75.75 0 1 1 1.06 1.06l-7.425 7.425Z" clipRule="evenodd" />
													</svg>
												) : null}
											</div>

											{voucherStatus[index] && voucherStatus[index] !== "¡Código válido!" && (
												<AppleErrorMessage voucher={true}>{voucherStatus[index]}</AppleErrorMessage>
											)}

											{voucherStatus[index] === "¡Código válido!" && index === couponCodes.length - 1 && couponCodes.length < Math.floor(getTotalBurgers() / 2) && addCouponField()}
										</div>
									))}
								</div>
							</div>
						...
						<div className="flex justify-center flex-col mt-6 items-center">
							...
							<div className="flex flex-row justify-between w-full">
								<div className="flex flex-row items-center gap-2">
									<Tooltip text={`Mismo valor a toda la ciudad, independientemente de que tan lejos estes.`} duration={10000} className="flex items-center" />
									<p className="font-coolvetica flex flex-row items-center gap-1">Envio</p>
								</div>
								<p>{values.deliveryMethod === "delivery" ? currencyFormat(envio) : currencyFormat(0)}</p>
							</div>
							<div className="flex flex-row justify-between w-full">
								<p className="font-coolvetica font-medium">Velocidad extra</p>
								<p>{isEnabled ? currencyFormat(expressDeliveryFee) : currencyFormat(0)}</p>
							</div>
							<div className="flex flex-row justify-between w-full">
								<p>Descuentos</p>
							...
							<div className="flex flex-row justify-between border-t border-opacity-20 border-black mt-4 pt-4 px-4 w-screen">
								...
								<p className="text-2xl font-bold">{currencyFormat(calculateFinalTotal())}</p>
							</div>
						</div>
						...
						{values.paymentMethod === "mercadopago" ? (
							...
							>
								{isSubmitting ? <LoadingPoints color="text-gray-100" /> : "Pedir"}
							</button>
						)}
					</div>
				</Form>
	...
	{/* Modales */}
	<AppleModal isOpen={isTimeRestrictedModalOpen} onClose={closeTimeRestrictedModal} title="Está cerrado">
	...
	>
		<p>Estamos en alta demanda, tu pedido comenzará a cocinarse dentro de {altaDemanda?.delayMinutes} minutos, ¿lo esperas?</p>
	</AppleModal>
</div>
);
};
