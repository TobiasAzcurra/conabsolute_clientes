...
const Reclamos = () => {
  ...
  const updateOrderWithComplaint = async (
    orderId,
    fecha,
    descripcionReclamo,
    alias
    ...
    if (!searchResults.length && hasSearched && !searching) {
      return (
        <p className="text-black text-center text-xs">
          No se encontraron pedidos en los últimos 3 días.
        </p>
      );
    }
    ...
    return (
      <div className="space-y-2">
        ...
        {ordersToShow.map((order) => (
          <div
            key={order.id}
            className={`bg-gray-50  p-4 rounded-3xl mx-4 border relative 
                            ${
            ...
            <div className="mt-6">
              ...
              >
                ...
                <img
                  src={arrow}
                  alt=""
                  className={`h-2 opacity-70 ml-2 ${
                    expandedOrders.has(order.id) ? "-rotate-90" : "rotate-90"
                  }`}
                />
              </button>
    ...
    return (
      <div className="bg-gray-50  px-4 min-h-screen justify-center font-coolvetica flex flex-col items-center">
        ...
        <p className="text-sm text-center text-gray-600">
          Damos lo mejor de nosotros pero a veces hay cosas que escapan de
          nuestro control y nos responsabilizamos cuando ocurre! Es nuestro
          deber con la gente que nos elige ❤️‍🩹
          <br />
          En breve te transfieren la compensacion y aca van unos regalos!
  ...
  return (
    <div className="bg-gray-50  py-4 min-h-screen justify-center font-coolvetica flex flex-col">
      ...
      >
        <img src={logo} className="w-2/3 flex mx-auto" alt="" />
        ...
        <form onSubmit={handleSubmit} className="w-full max-w-md">
          ...
          {selectedOrder && (
            <div className="flex flex-col px-4 mt-2">
              ...
              >
                {loading ? <LoadingPoints color="text-gray-100" /> : "Enviar"}
              </button>
            </div>
          )}
        </form>
      ...
      >
        <p className="text-center">
          Este pedido ya tiene un reclamo en curso. Por favor, espera a que se
          resuelva.
        </p>
      </AppleModal>
    </div>
